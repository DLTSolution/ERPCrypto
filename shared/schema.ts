...
export const collaterals = pgTable("collaterals", {
  ...
  // numeric with explicit TS type to keep numbers in code
  amount: numeric("amount").$type<number>().notNull(),
  valueUsd: numeric("value_usd").$type<number>().notNull(),
  chain: text("chain"),
  hash: text("hash"),
  txDate: text("tx_date"),
  feeToken: text("fee_token"),
  feeAmount: numeric("fee_amount").$type<number | null>(),
  feeValueUsd: numeric("fee_value_usd").$type<number | null>(),
  ptax: numeric("ptax").$type<number | null>(),
  totalValueBrl: numeric("total_value_brl").$type<number | null>(),
...
export const borrows = pgTable("borrows", {
  ...
  protocol: text("protocol"),
  asset: text("asset").notNull(),
  borrowedAmount: numeric("borrowed_amount").$type<number>().notNull(),
  interestRate: numeric("interest_rate").$type<number>().notNull(),
  valueUsd: numeric("value_usd").$type<number>().notNull(),
  chain: text("chain"),
  hash: text("hash"),
  txDate: text("tx_date"),
